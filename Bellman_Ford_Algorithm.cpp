...
void bellman_ford(int s,int N){
    ...
    for(int i=0; i< N-1; i++){ // Outer loop will run (N-1) times
        ...
        if(flag == false)//i.e. If relaxation did not take place atleast one time 
            break;
    }
}

void print_path(int desti){
    cout<<"\nPath to reach given node : ";
    while(desti!=-1){
        cout<<desti<<" ";
        desti=parent[desti];
    }
...
int main()
{
    ...
    int s;
    cin>>s;
    bellman_ford(s,N);//Passing number of nodes to run outer loop
